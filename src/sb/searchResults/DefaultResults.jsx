...
import { loadMoreButton, productFields, recommendations as RECOMMENDATIONS_DEFAULTS } from '../common/Defaults';
...
function DefaultResults({ loadMoreActive, setLoadMoreActive }) {
   ...
   function getDBOverlayTitle() {
      ...
      const configuredNameField = productFields.name;
      const { [configuredNameField]: name, title, uid } = overlayResult;

      return name?.trim().length ? name : title?.trim().length ? title : uid;
   }
   ...
   const contentTypeIsDB = ['db', 'csv', 'mongodb', 'product_discovery'].includes(currentContentType) || overlayResult.isStructured;
   ...
   return (
      <>
         <ul className={`${resultsStyles.resultsList} ${isGridView ? resultsStyles.grid : resultsStyles.list}`}>
            {
               RECOMMENDATIONS_DEFAULTS.enabled && recommendationsEnabled && <Recommendations />
            }
            {
